...
##############################################################################
open(INPUT, "$input_file") or die "Could not open INPUT $!\n";
open(OUTPUT, ">${input_file}.out") or die "Cannot open the record file for writing: $!\n";
...
sub decode_mrt_packet {
	...
	if ($type == MSG_TABLE_DUMP) { ###########################################
		...
		if ($subtype == INDEX_TABLE) {
			...
			$view_name ||= '';
			print OUTPUT "TYPE: MSG_TABLE_DUMP_V2/INDEX_TABLE\n".
			...
			while (length $rest > 0) {
				...
				print OUTPUT "      PEER $peer_index: ID: $bgp_id, " 
			...
			print OUTPUT "TYPE: MSG_TABLE_DUMP_V2/RIB_GENERIC\n"
				. "SEQUENCE: $seq_num\n"
		...
		while (length $rest > 0) {
			...
			if ($format == 1) {
				print OUTPUT "TYPE: MSG_TABLE_DUMP_V2/ENTRY_" .
						($af == AFI_IP ? 'AFI_IP' : 'AFI_IP6') . "\n"
					. "SEQUENCE: $seq_num\n"
					. 'PREFIX: ' . inet_ntop($af, $prefix) . "/$prefixlen\n"
					. "ORIGINATED: " . localtime($originated) . "\n";
				my $peer_as = $BGP_Peers[$peer_index]->[3];
				print OUTPUT 'FROM: ' . inet_ntop($af, $BGP_Peers[$peer_index]->[2])
					. " AS$peer_as\n";
				print_verbose_attributes($attr);
				print OUTPUT "-\n";
			} elsif ($format == 2) {
		...
		if ($subtype == BGP4MP_STATE_CHANGE) { #------------------------------
			...
			print OUTPUT "TYPE: BGP4MP/BGP4MP_STATE_CHANGE " .
					($af == AFI_IP ? 'AFI_IP' : 'AFI_IP6' ) . "\n";
			print OUTPUT "FROM: " . inet_ntop($af, $srcip) . "\n" if notnull($srcip);
			print OUTPUT "TO: "   . inet_ntop($af, $dstip) . "\n" if notnull($dstip);
			print OUTPUT "OLD STATE: $old_state  NEW STATE: $new_state\n";
			...
			print OUTPUT "TYPE: BGP4MP/BGP4MP_MESSAGE " .
					($af == AFI_IP ? 'AFI_IP' : 'AFI_IP6' ) . "\n";
			print OUTPUT "FROM: " . inet_ntop($af, $srcip) . "\n" if notnull($srcip);
			print OUTPUT "TO: "   . inet_ntop($af, $dstip) . "\n" if notnull($dstip);
			...
			print OUTPUT "TYPE: BGP4MP/BGP4MP_ENTRY "
				. ($afi == AFI_IP ? 'AFI_IP' : 'AFI_IP6' ) . "\n";
			print_verbose_attributes($attr);
		} else { #------------------------------------------------------------
			print OUTPUT "TYPE: BGP4MP/BGP4MP_UNKNOWN-$subtype\n";
			hexdump($$pkt);
		}
		...
		print OUTPUT "TYPE: TYPE: BGP4MP/BGP4"
			. ($afi == AFI_IP ? '' : 'PLUS') . "/UPDATE\n";
		print OUTPUT "FROM: " . inet_ntop($afi, $srcip) . " AS$srcas\n" if $srcas;
		print OUTPUT "TO: "   . inet_ntop($afi, $dstip) . " AS$dstas\n" if $dstas;
		print_verbose_attributes($attr);
		print OUTPUT "WITHDRAWN: $_\n" foreach (parse_nlri_prefixes($unf_routes));
		print OUTPUT "ANNOUNCE: $_\n" foreach (parse_nlri_prefixes($nlri));
	} else { ################################################################
	...
	while (length $attributes > 0) {
		...
		if ($type == BGP_ATTR_ORIGIN) {
			...
			while ($num_snpa-- > 0) {
				...
				print OUTPUT "|SNPA: "; hexdump($snpa); # XXX
			}

			if ($format == 1) { # XXX should not print OUTPUT here...
			print OUTPUT "|AFI: $afi ($safi)\n";
			print OUTPUT "|NEXT_HOP: " . inet6_ntoa($next_hop_global)
				. "  (LENGTH: $next_hop_len)\n";
...
sub parse_bgp_packet {
	...
	if		($type == BGP_TYPE_OPEN) {
		...
		# die if $version != 4;
		print OUTPUT "BGP PACKET TYPE: OPEN\n"
		...
		while (length $params > 0) {
			...
			if ($par_type == 1) {
				...
				print OUTPUT "PARAMETER: AUTH code $code\n";
			} elsif ($par_type == 2) {
				my $caps = $par_value;
				while (length $caps > 0) {
					...
					print OUTPUT "PARAMETER: CAPABILITY $cap_code\n";
				}
			} else {
				print OUTPUT "PARAMETER: TYPE $par_type (UNKNOWN)  "
					. "LEN: " . length($params) . "\n";
				hexdump($params);
			}
		}
	} elsif ($type == BGP_TYPE_UPDATE) {
		print OUTPUT "BGP PACKET TYPE: UPDATE\n";
		my ($unf_routes, $attributes, $nlri) = unpack('n/a n/a a*', $data);
		my $attr = parse_attributes($attributes);
		print_verbose_attributes($attr);
		print OUTPUT "WITHDRAWN: $_\n" foreach (parse_nlri_prefixes($unf_routes));
		print OUTPUT "ANNOUNCED: $_\n" foreach (parse_nlri_prefixes($nlri));
	} elsif ($type == BGP_TYPE_NOTIFICATION) {
		print OUTPUT "BGP PACKET TYPE: NOTIFICATION\n";
		my ($error, $suberror, $data) = unpack('C C a*', $data);
		print OUTPUT "BGP PACKET TYPE: ERROR $error (subcode $suberror)\n";
	} elsif ($type == BGP_TYPE_KEEPALIVE) {
		print OUTPUT "BGP PACKET TYPE: KEEPALIVE\n";
	} else { die }
}
...
sub print_verbose_attributes {
	...
	print OUTPUT 'ORIGIN: ' . $BGP_ORIGIN[$attr->[BGP_ATTR_ORIGIN]] . "\n"
		if $attr->[BGP_ATTR_ORIGIN];
	print OUTPUT 'AS_PATH:'  . print_aspath($attr->[BGP_ATTR_AS_PATH]) . "\n"
		if $attr->[BGP_ATTR_AS_PATH];
	print OUTPUT 'NEXT_HOP: ' . inet_ntoa($attr->[BGP_ATTR_NEXT_HOP])."\n"
		if notnull($attr->[BGP_ATTR_NEXT_HOP]);
	print OUTPUT 'MULTI_EXIT_DISC: ' .  unpack('N', $attr->[BGP_ATTR_MULTI_EXIT_DISC])
		. "\n" if $attr->[BGP_ATTR_MULTI_EXIT_DISC];
	print OUTPUT "ATOMIC_AGGREGATE\n" if $attr->[BGP_ATTR_ATOMIC_AGGREGATE];
	print OUTPUT 'AGGREGATOR: ' . unpack('n', ${$attr->[BGP_ATTR_AGGREGATOR]}[0])
